...
namespace Stand13
{
    public static class Tags
    {
        ...
        private static void OPCGroupDataChange(int TransactionID, int NumItems, ref Array ClientHandles, ref Array ItemValues, ref Array Qualities, ref Array TimeStamps)
        {
            ...
            {
                GetTagByClientHandle((int)ClientHandles.GetValue(i)).Value = ItemValues.GetValue(i).ToString();
            }
        }
        ...
        /// <summary>
        /// Добавляем теги в переданную группу
        /// </summary>
        /// <param name="group"></param>
        public static void AddTagsToOPCGroup(OPCGroup group)
        {
            group.DataChange += OPCGroupDataChange;
            foreach (Tag item in Items)
            {
                item.ServerHandle = group.OPCItems.AddItem(item.TagID, item.ClientHandle).ServerHandle;
            }
        }
    }
}
